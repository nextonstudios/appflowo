...
serve(async (req) => {
  ...
  try {
    ...
    const conceptosHTML = factura.conceptos.map((c: any) => `
      <tr>
        <td style="padding: 10px 16px; color: #FFFFFF; font-size: 13px; border-bottom: 1px solid #252B3B;">${c.descripcion}</td>
        <td style="padding: 10px 16px; color: #1DB8A0; font-size: 13px; font-weight: 600; text-align: right; border-bottom: 1px solid #252B3B;">$${c.monto.toLocaleString()}</td>
      </tr>
    `).join("");

    const tareasHTML = factura.tareas_realizadas && factura.tareas_realizadas.length > 0
      ? `
        <div style="margin-top: 24px;">
          ...
          ${factura.tareas_realizadas.map((t: string) => `
            <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 6px;">
              <span style="color: #1DB8A0; font-size: 12px;">✓</span>
              <span style="color: #6B7280; font-size: 13px;">${t}</span>
            </div>
          `).join("")}
        </div>
      ` : "";

    const abonoHTML = factura.abonado > 0 && factura.estado !== "pagada"
      ? `
        <div style="margin-top: 8px; padding: 12px 16px; background-color: #1A1F2E; border-radius: 8px;">
          <div style="display: flex; justify-content: space-between; margin-bottom: 4px;">
            ...
            <span style="color: #6B7280; font-size: 12px;">$${factura.abonado.toLocaleString()}</span>
          </div>
        ...
        <div style="background-color: #141824; padding: 28px 32px; border-bottom: 1px solid #252B3B;">
          <div style="display: flex; justify-content: space-between; align-items: flex-start;">
            ...
            <div style="text-align: right;">
              <p style="color: #FFFFFF; font-size: 14px; font-weight: 600; margin: 0 0 2px 0;">${factura.numero}</p>
              <p style="color: #6B7280; font-size: 11px; margin: 0;">Emisión: ${factura.fecha_emision}</p>
              <p style="color: #6B7280; font-size: 11px; margin: 0;">Vencimiento: ${factura.fecha_vencimiento}</p>
            </div>
          </div>
        </div>
        ...
        <div style="padding: 28px 32px;">
          <div style="display: flex; gap: 32px; margin-bottom: 24px;">
            <div>
              ...
              <p style="color: #FFFFFF; font-size: 14px; margin: 0;">${factura.cliente_nombre}</p>
            </div>
            <div>
              <p style="color: #6B7280; font-size: 11px; text-transform: uppercase; letter-spacing: 1px; margin: 0 0 4px 0;">Proyecto</p>
              <p style="color: #FFFFFF; font-size: 14px; margin: 0;">${factura.proyecto_nombre}</p>
            </div>
          </div>
          ...
          ${factura.notas ? `
            <div style="margin-top: 24px; padding: 14px 16px; background-color: #141824; border-radius: 8px; border-left: 3px solid #1DB8A0;">
              ...
              <p style="color: #FFFFFF; font-size: 13px; margin: 0; line-height: 1.5;">${factura.notas}</p>
            </div>
          ` : ""}
        </div>
    ...
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        to: [emailCliente],
        subject: "Factura " + factura.numero + " — " + factura.proyecto_nombre,
        html,
      }),
    });
