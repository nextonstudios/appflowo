...
const estadoConfig = {
  "pendiente": { label: "Pendiente", color: "text-muted bg-gray/10" },
  "en-progreso": { label: "En progreso", color: "text-violet bg-violet/10" },
  "completada": { label: "Completada", color: "text-accent bg-accent/10" },
};
      ...
      {estaEditando ? (
        <div className="flex flex-col gap-3">
          <div className="flex items-center justify-between">
            <p className="text-primary font-medium text-sm">Editar tarea</p>
            <button onClick={() => setEditandoTareaId(null)} className="text-muted text-xs hover:text-primary">Cancelar</button>
          </div>
          <input value={editTitulo} onChange={(e) => setEditTitulo(e.target.value)} placeholder="Titulo de la tarea"
          ...
          <div className="flex flex-wrap items-center gap-3">
              ...
              {(["alta", "media", "baja"] as const).map((p) => (
                <button key={p} type="button" onClick={() => setEditPrioridad(p)}
                  className={"text-xs px-3 py-1 rounded-md transition-colors font-medium " +
                    (editPrioridad === p ? prioridadConfig[p].color : "text-muted hover:text-primary")}>
            ...
            <button type="button" onClick={() => setEditPublica(!editPublica)}
              ...
              {editPublica ? "Visible al cliente" : "Oculta para el cliente"}
            </button>
          ...
          <div>
            ...
            <div className="space-y-1 mb-2">
              {editSubtareas.length === 0 && (
                <p className="text-muted text-xs">Sin subtareas.</p>
              )}
            ...
            <div className="flex-1 min-w-0">
              ...
              {!plegada && (
                <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-1">
                  ...
                  {tarea.publica && <span className="text-muted text-xs">👁 Visible</span>}
                  {tarea.aprobada_cliente && (
                    <span className="text-accent text-xs bg-accent/10 px-2 py-0.5 rounded-full font-medium">✓ Aprobada</span>
                  )}
                  {tarea.folder_url && (
                    <button onClick={() => openUrl(tarea.folder_url!)} className="text-muted text-xs hover:text-accent">📁 Carpeta</button>
                  )}
                </div>
              )}
            </div>
            <span className={"text-xs px-2.5 py-1 rounded-full font-medium flex-shrink-0 " + prioridadConfig[tarea.prioridad].color}>
            ...
            <>
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex gap-2">
                          <button onClick={() => onAgregarSubtarea(tarea.id)}
                            className="bg-accent text-onaccent font-medium px-3 py-1 rounded-lg text-xs hover:opacity-90">Guardar</button>
                          <button onClick={() => { setSubtareaAbiertaId(null); setNuevoTituloSubtarea(""); setNuevaSubtareaPublica(false); }}
                            className="text-muted px-2 py-1 rounded-lg text-xs hover:text-primary">Cancelar</button>
                        </div>
                      </div>
                    </div>
                  ) : null
              ...
              {confirmandoEliminar && (
                <div className="mt-3 ml-7 bg-coral/10 border border-coral/30 rounded-lg px-3 py-2 flex flex-wrap items-center justify-between gap-2">
                  <p className="text-coral text-xs">¿Eliminar esta tarea y sus subtareas?</p>
                  <div className="flex gap-2">
